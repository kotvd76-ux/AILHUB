const AI_EXERCISE_CONFIG = {
    ...
    getLevelInstruction(level) {
        const p = (typeof getExerciseLevelProfile === 'function')
            ? getExerciseLevelProfile(level)
            : { pace: 'Помірний навчальний темп', lexicon: 'розмовна лексика середнього рівня', grammar: 'основні часові й модальні форми', sentences: '2–3 речення по 7–11 слів', constraints: 'базові складнопідрядні' };
        return `Рівень учня: ${level}. Профіль: темп — ${p.pace}; лексика — ${p.lexicon}; граматика — ${p.grammar}; довжина/структура — ${p.sentences}; обмеження — ${p.constraints}. Суворо дотримуйся цього профілю.`;
    },
